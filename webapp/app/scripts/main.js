        ...
        $.getJSON(host + '/rail/view/' + searchName)
            ...
            L.geoJson(data, {
                pointToLayer: function (feature, latlng) {
                    ...
                    L.marker(new L.latLng(latlng.lat,latlng.lng-((10-(map.getZoom()*0.2)))), {icon: htmlIcon}).bindPopup(popupContent).addTo(railStationsInfoBoxes);
                    return L.marker(latlng).bindPopup(popupContent);
                }
            }).addTo(railStations);
            //console.log(map.getZoom());
            console.log(10-(map.getZoom()*0.8));
